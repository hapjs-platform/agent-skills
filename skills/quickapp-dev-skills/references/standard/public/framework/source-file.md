...
APP，页面和自定义组件均通过 ux 后缀文件编写，ux 后缀文件由[template 模板](template.md)、[style 样式](style-sheet.md)和[script 脚本](script.md)3 个部分组成，一个典型的页面 ux 后缀文件示例如下：
...
如果您希望捕获应用运行时异常, 可以在`<script>`中添加 onError 函数。当 app 运行发生异常时，异常信息将会通过 onError 回调通知您。
